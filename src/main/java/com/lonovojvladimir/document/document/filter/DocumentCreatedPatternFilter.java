...
@Component
public class DocumentCreatedPatternFilter implements DocumentFilter {
    @Override
    public boolean isApplicable(DocumentFilterDto documentFilterDto) {
        return documentFilterDto.getCreatedPattern()!=null;
    }

    @Override
    public Stream<DocumentEntity> apply(Stream<DocumentEntity> documentEntityStream, DocumentFilterDto documentFilterDto) {
        return documentEntityStream.filter(documentEntity -> documentEntity.getCreated().equals(documentFilterDto.getCreatedPattern()));
    }
}
